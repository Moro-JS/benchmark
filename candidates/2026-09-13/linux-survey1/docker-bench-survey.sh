...
docker run --rm --privileged --security-opt seccomp=unconfined \
  ...
  node:24-trixie bash -c '
    ...
    for G in wrk oha bombardier autocannon; do
      echo "################ generator=$G"
      node bench.js raw-engine-local engine-local raw-bun raw-uws --rate=20000 --keepalive=off --generator=$G --runs=2 --duration=10 --perf --save 2>&1 | grep -E "^\[|=> |run [0-9]/2:|best:|Saved:|WARNING|skipp|stderr|NOTE|perf|instr|syscall"
    done
    cp -f results-*.json results-*.md /out/ 2>/dev/null; ls /out
  ' > "$OUT/bench-linux-survey.log" 2>&1
echo "bench-linux-survey exit $?"

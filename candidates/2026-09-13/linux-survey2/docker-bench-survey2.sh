...
docker run --rm --privileged --security-opt seccomp=unconfined \
  ...
  node:24-trixie bash -c '
    ...
    # Three rounds with the row order rotated, so a drifting VM does not
    # favour whichever row runs first; the tables are compared per round.
    for ROUND in 1 2 3; do
      case $ROUND in 1) ROWS="raw-engine-local engine-local raw-bun raw-uws";; 2) ROWS="raw-uws raw-bun engine-local raw-engine-local";; 3) ROWS="raw-bun raw-engine-local raw-uws engine-local";; esac
      for G in wrk oha bombardier autocannon; do
        echo "################ round=$ROUND generator=$G"
        node bench.js $ROWS --rate=20000 --keepalive=off --generator=$G --runs=2 --duration=10 --perf --save 2>&1 | grep -E "^\[|=> |best:|Saved:|WARNING|skipp|stderr|NOTE"
      done
    done
    cp -f results-*.json results-*.md /out/ 2>/dev/null; ls /out
  ' > "$OUT/bench-linux-survey2.log" 2>&1
echo "bench-linux-survey2 exit $?"

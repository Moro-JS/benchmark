#!/usr/bin/env bash
# Connection-churn cross-check with oha (--disable-keepalive), the generator
...
docker run --rm --security-opt seccomp=unconfined \
  ...
  node:24-trixie bash -c '
    ...
    node bench.js raw-engine-local engine-local cluster-local raw-bun elysia-bun raw-uws --keepalive=off --generator=oha --runs=3 --duration=15 --save 2>&1 | grep -E "^\[|=> |run [0-9]/3:|best:|Saved:|WARNING|skipp|stderr|threads|workers"
    cp -f results-*.json results-*.md /out/ 2>/dev/null; ls /out
  ' > "$OUT/bench-linux-churn-oha.log" 2>&1
echo "bench-linux-churn-oha exit $?"

...
engine's way: the responses are not equal-sized (engine 149 bytes — it
sends `Connection: keep-alive`; uWS 142 bytes, sending a `uWebSockets: 20`
header instead), so at equal rps the engine is moving ~5% more bytes per
second through the same syscalls. The **pipelined ordering is outside the
